...
build_select_query(?DEGREE_FRIEND_2, JIDString, Username, SinceString)->
	...
	"SELECT confessions.*, GROUP_CONCAT(confession_favorites.jid SEPARATOR ', ') AS favorited_users, count(confession_favorites.jid) AS num_favorites FROM confessions LEFT JOIN confession_favorites ON confessions.confession_id = confession_favorites.confession_id WHERE confessions.jid IN (SELECT DISTINCT username FROM rosterusers WHERE jid IN (SELECT jid FROM rosterusers WHERE username = '" ++ Username ++ "')) AND username NOT IN (SELECT username FROM rosterusers WHERE jid = '" ++ JIDString ++ "') AND confessions.jid != '" ++ Username ++ "' GROUP BY confessions.confession_id ORDER BY confessions.created_timestamp ASC LIMIT 100";
...
build_select_query(?DEGREE_FRIEND_3, JIDString, Username, SinceString)->
	...
	"FROM confessions LEFT JOIN confession_favorites ON confessions.confession_id = confession_favorites.confession_id WHERE confessions.jid IN " ++
 	...
 	"AND confessions.jid != '" ++ Username ++ "' " ++
	"GROUP BY confessions.confession_id " ++
	"LIMIT 25 ORDER BY created_timestamp ASC";
